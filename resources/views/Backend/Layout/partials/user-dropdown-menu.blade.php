...
<div class="user-dropdown-menu">
    <div class="dropdown open">
        <a  @class(['btn', 'dropdown-toggle' => !isset($is_phone)]) type="button" id="{{$id}}" data-bs-toggle="dropdown" aria-haspopup="true"
                aria-expanded="false">
                    <img src="{{ $auth_->avatar_url ?? '' }}" alt="{{ $auth_->avatar_url ?? '' }}" class="avatar-30 @if (!isset($is_phone)) me-1 @endif">
                    @if (!isset($is_phone))
                        <span>{{ $auth_->name ?? '' }}</span>
                    @endif
                </a>
        <div class="dropdown-menu" aria-labelledby="{{$id}}">
            <a class="dropdown-item" href="#"><i class="bi bi-person me-2"></i> @lang('Profile')</a>
